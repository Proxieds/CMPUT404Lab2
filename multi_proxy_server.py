    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_server:
        ...
        while True:
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as proxy_client:
                ...
                # Start a process daemon for handling multiple connections with proxy client sockets
                p = Process(target=handle_request, args=(proxy_client, conn))
                p.daemon = True
                p.start()
                print("Started process ", p)

def handle_request(proxy_client, conn):
    # Send data to google
